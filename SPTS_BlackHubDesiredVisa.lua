-- ESP has been removed due to it breaking the esp in Horizon

if game.PlaceId ~= 2202352383 then return end
for i, v in pairs(game.CoreGui:GetChildren()) do
    if v.Name == "MainGUI" then
        v:Destroy()
    end
end
...
DeathReturn.Size = UDim2.new(0, 165, 0, 20)
...
PlayerInfo.Position = UDim2.new(0, 380, 0, 5)
PlayerInfo.Size = UDim2.new(0, 120, 0, 20)
PlayerInfo.TextColor3 = Color3.new(1, 1, 1)
PlayerInfo.Font = Enum.Font.Fantasy
PlayerInfo.Text = "Top Players Stats"
...
StatsFrame.Position = UDim2.new(0.0025, 0, 0.35, 0)
StatsFrame.Size = UDim2.new(0, 180, 0, 205)
StatsFrame.Active = true
StatsFrame.Selectable = true
StatsFrame.Draggable = true
StatsFrame.Visible = false

StatsFrameCorner.CornerRadius = UDim.new(0, 8)
StatsFrameCorner.Name = "UICorner"
StatsFrameCorner.Parent = StatsFrame

PlayerNameText.Name = "PlayerName"
PlayerNameText.Parent = StatsFrame
PlayerNameText.BackgroundColor3 = Color3.new(0.1, 0.1, 0.1)
PlayerNameText.BackgroundTransparency = 1
PlayerNameText.Position = UDim2.new(0.38, 0, -0.165, 0)
PlayerNameText.Size = UDim2.new(0, 50, 0, 90)
PlayerNameText.Font = Enum.Font.Fantasy
PlayerNameText.TextColor3 = Color3.new(1, 1, 1)
PlayerNameText.Text = " "
PlayerNameText.TextSize = 15
...
ShowStats1.Position = UDim2.new(0, 0, 0.39, 0)
ShowStats1.Size = UDim2.new(0.4, 0, 0.32, 0)
...
ShowStats2.Position = UDim2.new(0.41, 0, 0.33, 0)
...
spawn(function()
	while true do
        if not game.CoreGui:FindFirstChild("MainGUI") then break end
		if farmbtsafetyactive then
...
spawn(function()
	while true do
		...
		if playerdied then
			...
			repeat task.wait(0.1) until game.Players.LocalPlayer.Character.Humanoid
...
spawn(function()
	while true do
		...
		StatsFrame.Visible = false
		if playerdied == true then repeat task.wait(0.5) until playerdied == false end
        for i,v in pairs(game.Players:GetChildren()) do
			if(string.sub(string.lower(v.Name),1,string.len(statplayer))) == string.lower(statplayer) then
                ...
                local PlayerRep = converttoletter(string.format("%.0f", game.Players[v.Name].PrivateStats.Reputation.Value))
                local PlayerInnocents = converttoletter(string.format("%.0f", game.Players[v.Name].PrivateStats.InnocentKilled.Value))
                local PlayerVillains = converttoletter(string.format("%.0f", game.Players[v.Name].PrivateStats.VillainKilled.Value))
                local PlayerHeros = converttoletter(string.format("%.0f", game.Players[v.Name].PrivateStats.HeroKilled.Value))
                local PlayerTokens = converttoletter(string.format("%.0f", game.Players[v.Name].PrivateStats.Token.Value))
                local PlayerAliveT = converttoletter(string.format("%.0f", game.Players[v.Name].PrivateStats.AliveTime.Value))
                PlayerNameText.Text = game.Players[v.Name].Name
